...
@section('content')
    ...
    <div class="offcanvas offcanvas-end" id="add-new-record" style="width : 700px !important">
        ...
        <div class="offcanvas-body flex-grow-1">
            <form class="add-new-record pt-0 row g-3" id="form-labor" onsubmit="return false">
                @csrf
        ...
        $(document).ready(function() {
            ...
            var FDataTable = $('#FDataTable').DataTable({
                ...
                buttons: [{
                    ...
                    {
                        text: '<i class="mdi mdi-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Tambah Labor</span>',
                        className: 'create-new btn btn-primary me-2'
                    },
                ],

            });
            $('div.head-label').html('<h5 class="card-title mb-0">Data Labor</h5>')

            var LaborForm = {
                'form': $('#form-labor'),
                'insertBtn': $('#form-labor').find('#insertBtn'),
                'updateBtn': $('#form-labor').find('#updateBtn'),
                'id': $('#form-labor').find('#id'),
                'name': $('#form-labor').find('#name'),
                'address': $('#form-labor').find('#address'),
                'phone': $('#form-labor').find('#phone'),
                'email': $('#form-labor').find('#email'),
                'category': $('#form-labor').find('#category'),
            }
            ...
            function renderLabor(data) {
                ...
                Object.values(data).forEach((user) => {
                    ...
                    renderData.push([user['index'], button, user[
                            'name'],
                        user['address'],
            ...
            $('.create-new').on('click', function() {
                LaborForm.form.trigger('reset')
                LaborForm.updateBtn.attr('style', 'display: none !important');
                LaborForm.insertBtn.attr('style', 'display: ""');
                offCanvasEl.show();
            })

            FDataTable.on('click', '.edit', function() {
                var currentData = dataLabor[$(this).data('id')];
                LaborForm.form.trigger('reset')
                LaborForm.insertBtn.attr('style', 'display: none !important');
                LaborForm.updateBtn.attr('style', 'display: ""');
                offCanvasEl.show();
                LaborForm.id.val(currentData['id']);
                LaborForm.name.val(currentData['name']);
                LaborForm.address.val(currentData['address']);
                LaborForm.phone.val(currentData['phone']);
                LaborForm.email.val(currentData['email']);

            });

            LaborForm.insertBtn.on('click', () => {
                event.preventDefault();
                submit_form('{{ route('labor.create') }}', 'POST');
            });
            LaborForm.updateBtn.on('click', () => {
                event.preventDefault();
                submit_form('{{ route('labor.update') }}', 'PUT');
            });
                ...
                Swal.fire(SwalOpt()).then((result) => {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            // var user = data['data']
                            // dataLabor[user['id']] = user;
                            getAllLabor()
                            swalBerhasil();
                            offCanvasEl.hide();
                            renderLabor(dataLabor);
                            // LaborForm.self.modal('hide');
                        },
                        error: function(e) {}
                    });
                });
