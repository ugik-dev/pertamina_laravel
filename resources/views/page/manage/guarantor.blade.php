...
@section('content')
    ...
    <div class="offcanvas offcanvas-end" id="add-new-record" style="width : 700px !important">
        ...
        <div class="offcanvas-body flex-grow-1">
            <form class="add-new-record pt-0 row g-3" id="form-guarantor" onsubmit="return false">
                @csrf
        ...
        $(document).ready(function() {
            ...
            var FDataTable = $('#FDataTable').DataTable({
                ...
                buttons: [{
                    ...
                    {
                        text: '<i class="mdi mdi-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Tambah Guarantor</span>',
                        className: 'create-new btn btn-primary me-2'
                    },
                ],

            });
            $('div.head-label').html('<h5 class="card-title mb-0">Data Guarantor</h5>')

            var GuarantorForm = {
                'form': $('#form-guarantor'),
                'insertBtn': $('#form-guarantor').find('#insertBtn'),
                'updateBtn': $('#form-guarantor').find('#updateBtn'),
                'id': $('#form-guarantor').find('#id'),
                'name': $('#form-guarantor').find('#name'),
                'address': $('#form-guarantor').find('#address'),
                'phone': $('#form-guarantor').find('#phone'),
                'email': $('#form-guarantor').find('#email'),
                'category': $('#form-guarantor').find('#category'),
            }
            ...
            swalLoading();
            $.when(
                getAllGuarantor(), ).then((e) => {
                Swal.close();
            }).fail((e) => {
                console.log(e)
            });

            function getAllGuarantor() {
                return $.ajax({
                    url: `{{ route('guarantor.get') }}`,
                    'type': 'get',
                    ...
                    success: function(data) {
                        ...
                        dataGuarantor = data['data'];
                        renderGuarantor(dataGuarantor);
                    },
                    error: function(e) {}
                });
            }

            function renderGuarantor(data) {
                console.log(data)
                ...
                Object.values(data).forEach((user) => {
                    ...
                    renderData.push([user['index'], button,
            ...
            $('.create-new').on('click', function() {
                GuarantorForm.form.trigger('reset')
                GuarantorForm.updateBtn.attr('style', 'display: none !important');
                GuarantorForm.insertBtn.attr('style', 'display: ""');
                offCanvasEl.show();
            })

            FDataTable.on('click', '.edit', function() {
                var currentData = dataGuarantor[$(this).data('id')];
                GuarantorForm.form.trigger('reset')
                GuarantorForm.insertBtn.attr('style', 'display: none !important');
                GuarantorForm.updateBtn.attr('style', 'display: ""');
                offCanvasEl.show();
                GuarantorForm.id.val(currentData['id']);
                GuarantorForm.name.val(currentData['name']);
                // GuarantorForm.category.val(currentData['category']).trigger("change");
                // GuarantorForm.address.val(currentData['address']);
                // GuarantorForm.phone.val(currentData['phone']);
                // GuarantorForm.email.val(currentData['email']);
            });

            GuarantorForm.insertBtn.on('click', () => {
                event.preventDefault();
                submit_form('{{ route('guarantor.create') }}', 'POST');
            });
            GuarantorForm.updateBtn.on('click', () => {
                event.preventDefault();
                submit_form('{{ route('guarantor.update') }}', 'PUT');
            });
            ...
            function submit_form(url, metode) {
                Swal.fire(SwalOpt()).then((result) => {
                    ...
                    $.ajax({
                        url: url,
                        'type': metode,
                        data: GuarantorForm.form.serialize(),
                        success: function(data) {
                            if (data['error']) {
                            ...
                            getAllGuarantor()
                            // var user = data['data']
                            // dataGuarantor[user['id']] = user;
                            swalBerhasil();
                            offCanvasEl.hide();
                            renderGuarantor(dataGuarantor);
                            // GuarantorForm.self.modal('hide');
                        },
                        error: function(e) {}
                    });
                });
