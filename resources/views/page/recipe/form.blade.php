...
@section('content')
    ...
    <h4 class="py-3 mb-4">
        @if (!empty($id))
            <span class="text-muted fw-light">Recipe /</span>
            Form Edit
            {{-- <a class="btn btn-primary float-end ms-2" href="{{ route('detail-recipe', $dataContent->id) }}"> <i
        ...
        <a class="btn btn-primary float-end ms-2"
            href='{{ !empty($id) ? route('detail-recipe', $dataContent->id) : (!empty($dataForm->request_call_id) ? route('detail-recipe', $dataForm->request_call_id) : route('rekap.tindakan')) }}';>
            ...
            <div class="col-lg-3 col-md-4 col-12 mb-md-0 mb-3  order-sm-last">
                <div class="d-flex justify-content-between flex-column mb-2 mb-md-0">
                    ...
                    <button class="btn btn-warning float-end mb-2" id="saveBtn"> <i
                        ...
                        Save</button>
                    @include('page.recipe.form_partials.menu')
            ...
            <div class="col-lg-9 col-md-8 col-12 order-sm-first">
                <form class="needs-validation add-new-record pt-0 row g-3" id="form-tindakan" novalidate>
                    ...
                    <div class="tab-content p-0">
                        @include('page.recipe.form_partials.tap_1_informasi')
                        @include('page.recipe.form_partials.tap_3_recipe')
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="row mt-4">
            ...
            {{-- <div class="col-md-6 order-sm-last">
                <form class="needs-validation add-new-record pt-0 row g-3" id="form-tindakan" novalidate>
                    ...
                    <div class="tab-content p-0">
                        @include('page.recipe.form_partials.tap_1_informasi')
                        @include('page.recipe.form_partials.tap_2_waktu')
                        @include('page.recipe.form_partials.tap_3_keluhan')
                        @include('page.recipe.form_partials.tap_4_primary')
                        @include('page.recipe.form_partials.tap_5_secondary')
                    </div>
                </form>
            </div> --}}
    ...
    <script>
        ...
        function initSelect2Drug(elementId) {
            $(`#drugId_${elementId}`).select2({
                // dropdownParent: $('#accordionInformasi-Umum'),
                ajax: {
                    url: '<?= route('select2.drug') ?>',
                    type: "get",
                    dataType: 'json',
                    delay: 250,
                    data: function(params) {
            ...
            $(`#drugId_${elementId}`).on('select2:select', function(e) {
                var data = e.params.data;
                $(`#drugSatuan_${elementId}`).html(data.satuan)
                console.log(data);
            });
        }
        ...
        $(document).ready(function() {
            ...
            var TindakanForm = {
                ...
                'waktu_recipe': $('#form-tindakan').find('#waktu_recipe'),
            ...
            function addNewItem(number = '', qyt = '', signatura = '', drug_name = '', drug_id = '', cur_id = '') {
                drugId++;
                if (drug_id != '') {
                    option = `<option selected value="${drug_id}">${drug_name}</option>`
                } else {
                    option = ''
                }
                html = `
                ...
                <div class="row mt-2">
                    <div class="col-lg-9">
                         <select class="form-control" name="drug_id[]" id="drugId_${drugId}" class="form-control">
                         ${option}</select>
                    </div>

                    <div class="col-lg-3">
                        <div class=" input-group">
                            <input type="text" class="form-control" id="drugQyt_${drugId}" name="drug_qyt[]" value="${qyt}">
                            <div class="input-group-append">
                                <span class="input-group-text" id="drugSatuan_${drugId}">-</span>
                            </div>
                        </div>
                     </div>
                     </div>
                    <div class="row mt-2 mb-1">
                        <div class="col-lg-9">
                            <div class="form-group row">
                                <label for="signatura" class="col-lg-1 col-form-label">S</label>
                                <div class="col-lg-11">
                                <input type="text"  class="form-control" name="signatura[]" id="signatura_${drugId}" value="${signatura}">
                                </div>
                            </div>    
                        </div>
                        ...
                        <div class="col-lg-3">
                            <div class="form-group row">
                                <label for="drugNumber_" class="col-lg-2 col-form-label">No</label>
                                <div class="col-lg-10">
                                <input type="text"  class="form-control" name="drug_number[]" id="drugNumber_${drugId}" value="${number}">
                            </div>
                        </div>   
                     </div>
                </div>
                <div class="row">
                    <div class="col-lg-12">
                        <button type="button" class="btn btn-danger btn-sm remove-item" data-id="${drugId}">Hapus</button>
                    </div>
                </div>
                ...
                initSelect2Drug(drugId)
                $('.remove-item').on("click", function() {
                    console.log("delete")
                    const id = $(this).data('id');
                    $('#itemRow_' + id).remove(); // Hapus item dari DOM
                });
            }
            <?php if (!empty($dataForm)) : ?>
            <?php foreach ($dataForm->drugs as $drug): ?>
            addNewItem("{{ $drug->drug_number }}", "{{ $drug->drug_qyt }}", "{{ $drug->signatura }}",
                "{{ $drug->drug->name }}", "{{ $drug->drug->id }}", "{{ $drug->id }}");
            <?php endforeach; ?>
            <?php endif; ?>
