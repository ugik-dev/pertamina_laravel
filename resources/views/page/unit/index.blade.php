...
@section('content')
    ...
    <div class="offcanvas offcanvas-end" id="add-new-record" style="width : 700px !important">
        ...
        <div class="offcanvas-body flex-grow-1">
            <form class="add-new-record pt-0 row g-3" id="form-unit" onsubmit="return false">
                @csrf
        ...
        $(document).ready(function() {
            ...
            var FDataTable = $('#FDataTable').DataTable({
                ...
                buttons: [{
                    ...
                    {
                        text: '<i class="mdi mdi-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Tambah Unit</span>',
                        className: 'create-new btn btn-primary me-2'
                    },
                ],

            });
            $('div.head-label').html('<h5 class="card-title mb-0">Data Unit</h5>')

            var UnitForm = {
                'form': $('#form-unit'),
                'insertBtn': $('#form-unit').find('#insertBtn'),
                'updateBtn': $('#form-unit').find('#updateBtn'),
                'id': $('#form-unit').find('#id'),
                'name': $('#form-unit').find('#name'),
                'address': $('#form-unit').find('#address'),
                'phone': $('#form-unit').find('#phone'),
                'email': $('#form-unit').find('#email'),
                'category': $('#form-unit').find('#category'),
            }
            ...
            $('.create-new').on('click', function() {
                UnitForm.form.trigger('reset')
                UnitForm.updateBtn.attr('style', 'display: none !important');
                UnitForm.insertBtn.attr('style', 'display: ""');
                offCanvasEl.show();
            })

            FDataTable.on('click', '.edit', function() {
                var currentData = dataUnit[$(this).data('id')];
                UnitForm.form.trigger('reset')
                UnitForm.insertBtn.attr('style', 'display: none !important');
                UnitForm.updateBtn.attr('style', 'display: ""');
                offCanvasEl.show();
                UnitForm.id.val(currentData['id']);
                UnitForm.name.val(currentData['name']);
                UnitForm.category.val(currentData['category']).trigger("change");
                UnitForm.address.val(currentData['address']);
                UnitForm.phone.val(currentData['phone']);
                UnitForm.email.val(currentData['email']);

            });

            UnitForm.insertBtn.on('click', () => {
                event.preventDefault();
                submit_form('{{ route('unit.create') }}', 'POST');
            });
            UnitForm.updateBtn.on('click', () => {
                event.preventDefault();
                submit_form('{{ route('unit.update') }}', 'PUT');
            });
